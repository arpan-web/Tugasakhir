...
@section('content')
    <div class="container-fluid">
        <div class="mb-4">
            <a href="{{ route('pendaftaran.index') }}" class="btn btn-outline-secondary"><i data-feather="arrow-left"></i>
                Kembali</a>
        </div>
                ...
                <form action="{{ route('pendaftaran.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form-text">Pasien belum terdaftar? <a href="{{ route('pasien.create') }}">Tambah
                                    Pasien Baru</a></div>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.1" class="form-control" id="suhu_tubuh" name="suhu_tubuh"
                                value="{{ old('suhu_tubuh') }}" placeholder="Contoh: 36.5">
                        </div>

                        <div class="col-md-4">
                            <label for="berat_badan" class="form-label">Berat Badan (kg)</label>
                            <input type="number" step="0.1" class="form-control" id="berat_badan" name="berat_badan"
                                value="{{ old('berat_badan') }}" placeholder="Contoh: 65">
                        </div>

                        <div class="col-md-4">
                            <label for="tinggi_badan" class="form-label">Tinggi Badan (cm)</label>
                            <input type="number" step="0.1" class="form-control" id="tinggi_badan" name="tinggi_badan"
                                value="{{ old('tinggi_badan') }}" placeholder="Contoh: 170">
                        </div>

                        <!-- Keluhan -->
                        <div class="col-12 mt-3">
                            <label for="keluhan" class="form-label fw-bold">Keluhan Pasien</label>
                            <textarea class="form-control" id="keluhan" name="keluhan" rows="3" required
                                placeholder="Jelaskan keluhan utama pasien sakit apa...">{{ old('keluhan') }}</textarea>
                        </div>
                    </div>

                    <div class="mt-4 text-end">
                        <button type="reset" class="btn btn-light me-2">Reset Form</button>
                        <button type="submit" class="btn btn-primary"><i data-feather="check-circle" class="me-1"></i>
                            Daftarkan Antrian</button>
                    </div>
                </form>
            </div>
        </div>
